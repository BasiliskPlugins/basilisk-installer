...
import org.basilisk.utilities.DownloadUtils;
import org.basilisk.utilities.FileUtilities;
...
public class Main {
    ...
    public static void main(String[] args) throws IOException {
        ...
        loadingWindow.setStatusText("Checking Basilisk Version");

        String basiliskUrl = "https://github.com/BasiliskPlugins/basilisk-launcher/releases";
        String basiliskVersion = getLatestVersionString(basiliskUrl);
        println("Latest Basilisk Version: " + basiliskVersion);

        File[] files = FileUtilities.getSideLoadedPluginPath().listFiles();

       println("Checking for Basilisk Plugin");
        ...
        for (File file : files) {
            if (file.getName().contains("basilisk-launcher")) {
                found = true;
                println("Found Basilisk Plugin: " + file.getName());
                String[] split = file.getName().split("-");
                String version = split[split.length - 1].replace(".jar", "");
                println("Found Basilisk Plugin Version: " + version);

                if (!version.equals(basiliskVersion)) {
                    println("Found Basilisk Plugin Version: " + version + " does not match latest version: " + basiliskVersion);
                    println("Deleting old version");
                    loadingWindow.setStatusText("Basilisk out of date, deleting old version");
                    if (file.delete()) {
        ...
        if (!found) {
            println("Downloading latest Basilisk Plugin");
            loadingWindow.setStatusText("Downloading latest Basilisk Plugin");
            String latestVersionURL = getLatestVersionURL(basiliskVersion);
            println("Downloading from: " + latestVersionURL);
            String downloadPath = FileUtilities.getSideLoadedPluginPath().getPath() + File.separator + "basilisk-launcher-" + basiliskVersion + ".jar";
            DownloadUtils.downloadFile(latestVersionURL, downloadPath, loadingWindow);
            foundVersion = basiliskVersion; // Update the found version to the latest one.
        }
        ...
        if (foundVersion != null) {
            ...
            System.out.println("Error: Basilisk Plugin not found or downloaded.");
            System.exit(1);
        }
    }
    ...
    private static String getLatestVersionURL(String latestversion) {
        return String.format("https://github.com/BasiliskPlugins/basilisk-launcher/releases/download/v%s/basilisk-launcher-%s.jar", latestversion,latestversion);
    }
